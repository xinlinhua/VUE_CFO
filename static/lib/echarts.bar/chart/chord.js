define('echarts/chart/chord', [
    ...
    Chord.prototype = {
        ...
        _initHoverHandler: function (series, graphs) {
        	var selectedList = []; //存放所有激活的点
            var mainSerie = series[0];
            var mainGraph = graphs[0];
            var self = this;
            mainGraph.eachNode(function (node) {
            	node.shape.onclick = function () { 
            		var sameFlag = false; //判断是否取消激活点
            		for(var f=0; f<selectedList.length;f++){
            			if(selectedList[f].id === node.id){ //表示取消激活点
            				selectedList.splice(f, 1);
            				sameFlag = true;
            				break;
            			}
            		}
            		
            		//不是取消激活点操作，就把该点放入selectedList
            		if(!sameFlag){
            			selectedList.push(node);
            			selectedList = _.uniq(selectedList); //去重 
            		}
            		
                    mainGraph.eachNode(function (n) {
                        n.shape.style.opacity = 0.2;
                        if (n.labelShape) {
                            n.labelShape.style.opacity = 0.8; //点得名称不能太暗，ie下看不清
                    ...
                    for (var i = 0; i < graphs.length; i++) {
                        for (var j = 0; j < graphs[i].edges.length; j++) {
                            ...
                            e.shape.style.opacity = self.deepQuery(queryTarget, 'opacity') * 0.2;
                            e.shape.modSelf();
                        }
                    }
                    ...
                    _.each(selectedList, function(item){
	                    ...
	                    if (item.labelShape) {
	                        item.labelShape.style.opacity = 1;
	                    }
                    });                    
                    
                    for (var i = 0; i < graphs.length; i++) {
                    	_.each(selectedList, function(b){
                    		var n = graphs[i].getNodeById(b.id);
	                        if (n) {
	                            for (var j = 0; j < n.outEdges.length; j++) { //遍历所有和该点有关系的边
	                            	_.each(selectedList, function(a){
	                            		if(a.id === n.outEdges[j].data.target){ //只过滤出属于点击过的点的边，其他的不变亮
	                            			var e = n.outEdges[j];
	                                		var queryTarget = self._getEdgeQueryTarget(graphs[i].__serie, e.data);
